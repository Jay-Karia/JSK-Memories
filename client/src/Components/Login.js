...
const Login = (props) => {
    ...
    const handleSubmit = async (e)=> {
        ...
        if (emailValidate!==null && passwordValidate!==null && cPassValidate!==false) {
            ...
            props.showAlert('Could not Submit form', 'error')
        }
    }
                ...
                <div style={{border:'2px solid white', marginTop:"20px", marginBottom:'20px', backgroundColor:"white", borderRadius:'14px'}}>
                    ...
                    <Box component="form" sx={{ '& .Mu  iTextField-root': { m: 1, width: '20rem' }, padding:"20px"}} noValidate autoComplete="off">
                        <div style={{display:'flex', alignItems:'center', justifyContent:'center'}}>
                            <TextField color='success' error={emailValidate===null?true:false} type='email' onChange={(e)=>{setEmail(e.target.value);validateEmail()}} helperText={emailValidate===null?'Invalid Email':''} className="email" label="Email" id="outlined-multiline-static" style={{width:'25rem'}} defaultValue="" size="md"/>
                        </div>
                        <div style={{display:'flex', alignItems:'center', justifyContent:'center'}}>
                            <TextField error={passwordValidate===null?true:false} onChange={(e)=>{setPassword(e.target.value);validatePassword()}} type='password' color='success' helperText={passwordValidate===null?'Invalid Password':''} id="outlined-multiline-static " sx={{backgroundColor:'white', marginTop:'30px', width:'25rem'}} label="Password"/>
                        </div>
                        <div style={{display:'flex', alignItems:'center', justifyContent:'center'}}>
